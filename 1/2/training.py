...
def main():
    ...
    # Minimal GPU memory allocation (4 bytes)
    tensor_product = torch.tensor([1.0], device=device)
    
    try:
        print(tensor_product)
        print("Training initiated")
        while True:
            # Small non-trivial operation (prevents optimization)
            torch.neg(tensor_product, out=tensor_product)
            
            # Adjust sleep time based on your needs
            time.sleep(1)  # CPU sleeps, GPU gets brief activity
