...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        submodule_name=None,
        ortho_max=0.05,  # Reduced orthogonality weight
        base_threshold=0.6,  # More gradual start
        threshold_decay="quadratic",  # Quadratic decay
    ):
        super().__init__(seed)
        ...
        self.steps = steps
        self.k = k
        self.step = 0  # Track training progress
        self.ortho_max = ortho_max
    ...
    def orthogonality_penalty(self, activations, threshold):
        """Compute orthogonality penalty with smooth threshold."""
        ...
        # Apply threshold interpolation
        mask = torch.clamp((similarity - threshold)/threshold, 0, 1)
        # Zero out diagonal elements
        mask.fill_diagonal_(0)
        return torch.sum(torch.abs(similarity * mask))
    ...
    def loss(self, x, step=None, logging=False):
        ...
        # Calculate training progress and penalties
        p = self.step / self.steps
        # Quadratic decay for both threshold and lambda
        current_threshold = self.base_threshold * (1 - p)**2
        lambda_ortho = self.ortho_max * (1 - p)**2
        ...
        ortho_loss = lambda_ortho * self.orthogonality_penalty(f, current_threshold)
    ...
    def update(self, step, x):
        ...
        self.step = step  # Update current step
...
def run_sae_training(
    ...
    trainer = TrainerTopK(
        ...
        dict_class=AutoEncoderTopK,
        dict_size=dict_size,
        k=320,  # Restored baseline sparsity
        auxk_alpha=1/32,
...
if __name__ == "__main__":
 ...
 warmup_steps=1000,
  seed=42,
    ...
    eval_types = [
        "absorption",  # Track feature separation
        "core",
    ]
