...
class AutoEncoderTopK(nn.Module):
    ...
    def __init__(
        ...
        self.current_step = 0
        self.total_steps = 2441  # From baseline config
    ...
    def compute_orthogonality_loss(self, encoded_acts: torch.Tensor) -> torch.Tensor:
        """Compute progressive orthogonality loss"""
        progress = min(self.current_step / self.total_steps, 1.0)
        lambda_2 = self.lambda_2_max * (1 - progress) ** 2
        threshold = self.base_threshold * (1 - progress)
        
        # Compute normalized feature activations
        norms = torch.norm(encoded_acts, dim=0, keepdim=True)
        normalized_acts = encoded_acts / (norms + 1e-8)
        
        # Compute feature correlations
        correlations = torch.abs(torch.mm(normalized_acts.T, normalized_acts))
        
        # Mask for off-diagonal elements
        mask = 1 - torch.eye(correlations.shape[0], device=correlations.device)
        
        # Competition coefficients
        c_ij = torch.clamp((correlations - threshold) / threshold, min=0, max=1) * mask
        
        return lambda_2 * (c_ij * correlations * mask).sum()
    ...
    def forward(self, x: torch.Tensor, output_features: bool = False):
        ...
        if self.training:
            self.orthogonality_loss = self.compute_orthogonality_loss(encoded_acts)
        else:
            self.orthogonality_loss = torch.tensor(0.0, device=x.device)
...
class TrainerTopK(SAETrainer):
    ...
    def loss(self, x, step=None, logging=False):
        ...
        # Add orthogonality loss from forward pass
        loss = e.pow(2).sum(dim=-1).mean() + self.ae.orthogonality_loss
