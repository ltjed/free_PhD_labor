...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        self.logging_parameters = ["effective_l0", "dead_features", "ortho_loss", "progress", "threshold", "competition_coef"]
        self.effective_l0 = -1
    ...
    def loss(self, x, step=None, logging=False):
        ...
        # Calculate competition coefficients and thresholds
        base_threshold = 0.5  # Base threshold for competition
        threshold = base_threshold * (1 - progress)  # Decreasing threshold
        
        # Get normalized feature activations
        f_norm = f / (torch.norm(f, dim=1, keepdim=True) + 1e-8)
        
        # Compute feature correlation matrix
        feat_corr = torch.mm(f_norm, f_norm.t())
        # Zero out diagonal terms
        feat_corr = feat_corr - torch.diag(torch.diag(feat_corr))
        
        # Compute normalized mask intersection (n_ij)
        mask_intersect = (f > 0).float() @ (f > 0).float().t()
        mask_union = ((f > 0).float().sum(1, keepdim=True) + (f > 0).float().sum(1, keepdim=True).t() + 1e-8)
        n_ij = mask_intersect / mask_union
        
        # Compute competition coefficients
        c_ij = torch.clamp((n_ij - threshold) / threshold, 0, 1)
        
        # Compute weighted orthogonality loss
        ortho_loss = (c_ij * torch.abs(feat_corr)).mean()
        
        # Dynamic orthogonality weight
        lambda_2_max = 0.2  # Maximum orthogonality weight
        ortho_weight = lambda_2_max * (1 - progress) ** 2
        ...
        self.competition_coef = c_ij.mean().item()
        ...
        loss = l2_loss + self.auxk_alpha * auxk_loss + 0.1 * ortho_weight * ortho_loss
