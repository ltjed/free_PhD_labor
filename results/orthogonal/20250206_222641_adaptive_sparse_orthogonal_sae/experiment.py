...
class AutoEncoderTopK(nn.Module):
    """
    The top-k autoencoder architecture with progressive orthogonality constraints.
    """
    def __init__(
        self,
        d_in: int,
...
class TrainerTopK(SAETrainer):
    ...
    def compute_orthogonality_loss(self, encoded_acts, progress):
        """Compute orthogonality loss with progressive weighting"""
        # Get feature correlations
        feat_corr = torch.mm(encoded_acts.T, encoded_acts)
        feat_corr.diagonal().zero_()
        
        # Compute normalized mask intersection
        n_ij = feat_corr / (torch.norm(encoded_acts, dim=0).unsqueeze(0) * 
                           torch.norm(encoded_acts, dim=0).unsqueeze(1) + 1e-8)
        
        # Progressive threshold
        threshold = self.base_threshold * (1 - progress)
        
        # Competition coefficients
        c_ij = torch.clamp((n_ij - threshold) / threshold, min=0, max=1)
        
        # Progressive lambda
        lambda_2 = self.lambda_2_max * (1 - progress) ** self.decay_power
        
        return lambda_2 * (c_ij * torch.abs(feat_corr)).mean()

    def loss(self, x, step=None, total_steps=None, logging=False):
        # Run the SAE
        f, top_acts, top_indices = self.ae.encode(x, return_topk=True)
        
        # Store feature correlations for logging
        feat_corr = torch.mm(f.T, f) if logging else None
        ...
        ortho_loss = self.compute_orthogonality_loss(f, progress)
        if logging:
            feat_corr.diagonal().zero_()  # Zero out diagonal for overlap calculation
        loss = l2_loss + self.auxk_alpha * auxk_loss + ortho_loss

        if logging:
            # Log feature separation metrics
            with open(f"run_{step//100}/feature_separation_{step}.json", "w") as f:
                json.dump({
                    "progress": progress,
                    "orthogonality_loss": ortho_loss.item(),
                    "average_overlap": feat_corr.abs().mean().item()
                }, f)
    ...
    def update(self, step, x, total_steps=None):
        ...
        # compute the loss
        x = x.to(self.device)
        loss = self.loss(x, step=step, total_steps=total_steps)
        loss.backward()
...
MODEL_CONFIGS = {
    ...
    "google/gemma-2-2b": {"batch_size": 32, "dtype": "bfloat16", "layers": [12], "d_model": 16384},
}
