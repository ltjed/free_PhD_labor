...
class TrainerTopK(SAETrainer):
    ...
    def loss(self, x, step=None, logging=False):
        ...
        W_dec_norm = self.ae.W_dec / (self.ae.W_dec.norm(dim=1, keepdim=True) + 1e-8)
        cosine_sim = torch.mm(W_dec_norm, W_dec_norm.t())
        mask = torch.ones_like(cosine_sim) - torch.eye(cosine_sim.shape[0], device=cosine_sim.device)
        ortho_loss = (mask * cosine_sim.pow(2)).mean()
        
        # Dynamic orthogonality weight that decreases quadratically with progress
        lambda_ortho = 0.1 * (1 - progress) ** 2
        ...
        if logging and step is not None and step % 100 == 0:
            log_dir = os.path.dirname(os.path.abspath(__file__))
            log_file = os.path.join(log_dir, "feature_separation.json")
            ...
            log_data["progress"].append(float(progress))
            log_data["orthogonality_loss"].append(float(ortho_loss.item()))
            log_data["average_cosine_similarity"].append(float((mask * cosine_sim).mean().item()))
