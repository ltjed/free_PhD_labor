...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        base_threshold=0.8,  # base threshold for orthogonality
        lambda2_max=0.1,    # maximum lambda2 weight
        decay_rate=0.5,     # decay rate for progressive schedule
        seed=None,
        device=None,
        layer=None,
        lm_name=None,
    ...
    def compute_progress(self, step):
        """Compute training progress from 0 to 1"""
        return min(1.0, step / self.steps)

    def compute_threshold(self, progress):
        """Compute dynamic orthogonality threshold"""
        return self.base_threshold * (1 - progress)

    def compute_lambda2(self, progress):
        """Compute dynamic lambda2 weight"""
        return self.lambda2_max * (1 - progress)**2

    def compute_competition_coeffs(self, W_dec, threshold):
        """Compute competition coefficients between features"""
        # Compute normalized overlaps between decoder directions
        overlaps = torch.mm(W_dec, W_dec.t())
        # Zero out diagonal
        overlaps.fill_diagonal_(0)
        # Compute competition coefficients
        c_ij = torch.clamp((overlaps - threshold) / threshold, min=0)
        return c_ij

    def orthogonality_loss(self, W_dec, competition_coeffs):
        """Compute orthogonality loss term"""
        overlaps = torch.mm(W_dec, W_dec.t())
        overlaps.fill_diagonal_(0)
        return (competition_coeffs * overlaps**2).sum()
    ...
    def loss(self, x, step=None, logging=False):
        ...
        # Compute orthogonality loss with progressive schedule
        progress = self.compute_progress(step)
        threshold = self.compute_threshold(progress)
        lambda2 = self.compute_lambda2(progress)
        
        competition_coeffs = self.compute_competition_coeffs(self.ae.W_dec, threshold)
        ortho_loss = self.orthogonality_loss(self.ae.W_dec, competition_coeffs)
