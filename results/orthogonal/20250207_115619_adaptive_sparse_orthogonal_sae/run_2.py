...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        ortho_weight_max=5e-4,  # Maximum orthogonality constraint weight
        ...
        self.ortho_weight_max = ortho_weight_max
        ...
        self.ae =  AutoEncoderTopK(
            ...
            hook_layer=layer,
            model_name=lm_name,
            k = 320  # Default k value matching baseline
            )
    ...
    def loss(self, x, step=None, logging=False):
        ...
        W_dec = self.ae.W_dec
        dot_products = torch.mm(W_dec, W_dec.T)
        mask = torch.eye(dot_products.size(0), device=dot_products.device)
        ortho_loss = ((dot_products**2) * (1 - mask)).sum() / (self.ae.dict_size * (self.ae.dict_size - 1))
        
        # Calculate progress-based weighting
        p = step / self.steps if step is not None else 1.0
        lambda_ortho = self.ortho_weight_max * (1 - p)**2
    ...
    def config(self):
        return {
            ...
            "ortho_weight_max": self.ortho_weight_max,
        }
