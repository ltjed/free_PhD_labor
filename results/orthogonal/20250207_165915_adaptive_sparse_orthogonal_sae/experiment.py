...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        submodule_name=None,
        ortho_base_threshold=0.8,
        ortho_max_weight=0.1,
        ortho_decay_rate=1.0,
    ):
        super().__init__(seed)
        ...
        self.ortho_base_threshold = ortho_base_threshold
    ...
    def loss(self, x, step=None, logging=False):
        ...
        # Calculate training progress and orthogonality components
        p = step / self.steps if step is not None else 0
        threshold = self.ortho_base_threshold * (1 - p * self.ortho_decay_rate)
        lambda_ortho = self.ortho_max_weight * (1 - p)**2

        # Calculate feature co-activation matrix
        active_features = t.zeros((x.shape[0], self.ae.dict_size), device=x.device)
        active_features.scatter_(-1, top_indices, 1.0)
        n_ij = t.mm(active_features.T, active_features) / active_features.shape[0]

        # Competition coefficients
        c_ij = t.clamp((n_ij - threshold)/threshold, 0, 1)

        # Orthogonality loss
        W = self.ae.W_enc / t.norm(self.ae.W_enc, dim=0, keepdim=True)
        ortho_loss = t.sum(c_ij * t.abs(t.mm(W.T, W) - t.eye(W.shape[1], device=W.device)))

        # Combine all losses
        loss = l2_loss + self.auxk_alpha * auxk_loss + lambda_ortho * ortho_loss
    ...
    def config(self):
        return {
            ...
            "ortho_base_threshold": self.ortho_base_threshold,
            "ortho_max_weight": self.ortho_max_weight,
