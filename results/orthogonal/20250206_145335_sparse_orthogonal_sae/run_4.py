...
class AutoEncoderTopK(nn.Module):
    ...
    def compute_mask_intersections(self, indices_BK, threshold=0.5):
        ...
        # Normalize by batch size and apply threshold
        competition = intersect / B
        competition = torch.where(competition >= threshold, competition, torch.zeros_like(competition))
        return competition
...
class TrainerTopK(SAETrainer):
    ...
    def loss(self, x, step=None, logging=False):
        ...
        # Add orthogonality loss with weight 0.001
        loss = l2_loss + self.auxk_alpha * auxk_loss + 0.001 * ortho_loss
