...
class TemporalSAE(nn.Module):
    ...
    def __init__(
        ...
        # Add previous activation state tracking
        self.prev_acts = None
...
class TemporalTrainer(SAETrainer):
    ...
    def __init__(self,
        ...
        # Initialize tracking variables for temporal regularization
        self.max_recon_loss = None
    ...
    def resample_neurons(self, activations):
        with torch.no_grad():
            ...
            dead_threshold = 0.001 # Features active less than 1% of the time
    ...
    def loss(self, x, logging=False, **kwargs):
        ...
        if self.ae.prev_acts is not None:
            temporal_diff = (f - self.ae.prev_acts).abs().mean()
            # Adaptive scaling based on reconstruction quality
            alpha = torch.sigmoid(5.0 * (self.max_recon_loss - l2_loss)) if self.max_recon_loss is not None else 0.5
        ...
        if self.max_recon_loss is None:
            self.max_recon_loss = l2_loss.item()
        else:
            self.max_recon_loss = 0.99 * self.max_recon_loss + 0.01 * max(l2_loss.item(), self.max_recon_loss)
