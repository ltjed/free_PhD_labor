...
class TemporalSAE(nn.Module):
    ...
    def __init__(
        ...
        # Add state tracking for temporal regularization
        self.prev_acts = None
        self.max_recon_loss = None
        self.recon_loss_momentum = 0.99
...
class TemporalTrainer(SAETrainer):
    ...
    def __init__(self,
                 ...
                 lr=3e-4,
                 l1_penalty=0.04,
                 temporal_penalty=0.02,  # Initial temporal penalty
                 adaptation_rate=5.0,  # Adaptation rate β
                 warmup_steps=1000,
                 resample_steps=None,
                 seed=None,
                 device=None,
    ...
    def loss(self, x, logging=False, **kwargs):
        ...
        if self.ae.max_recon_loss is None:
            self.ae.max_recon_loss = l2_loss.item()
        else:
            self.ae.max_recon_loss = max(
                self.ae.max_recon_loss * self.ae.recon_loss_momentum,
                l2_loss.item()
            )
        
        # Compute adaptive scaling factor
        alpha = torch.sigmoid(self.adaptation_rate * (self.ae.max_recon_loss - l2_loss))
        
        # Compute temporal loss if previous activations exist
        temporal_loss = torch.tensor(0.0, device=x.device)
        if self.ae.prev_acts is not None:
            temporal_loss = torch.linalg.norm(f - self.ae.prev_acts, dim=-1).mean()
        ...
        loss = l2_loss + self.l1_penalty * l1_loss + \
               self.temporal_penalty * alpha * temporal_loss
...
MODEL_CONFIGS = {
    "EleutherAI/pythia-70m-deduped": {"batch_size": 512, "dtype": "float32", "layers": [3], "d_model": 16384},
