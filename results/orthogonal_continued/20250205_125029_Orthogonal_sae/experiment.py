...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        alpha=0.0,  # sharing coefficient for orthogonality
        lambda_2_max=0.1,  # max orthogonality weight
        t0=1000,  # orthogonality warmup steps
        ...
        # Add orthogonality metrics to logging
        self.logging_parameters += ["ortho_loss", "current_lambda2"]
    ...
    def loss(self, x, step=None, logging=False):
        ...
        W_enc = self.ae.W_enc  # [d_in, d_sae]
        W_dec = self.ae.W_dec  # [d_sae, d_in]
        enc_T_dec = t.mm(W_enc.T, W_dec)  # [d_sae, d_sae]
        
        identity = t.eye(enc_T_dec.size(-1), device=enc_T_dec.device)
        ortho_loss = t.norm(enc_T_dec - self.alpha * identity, p='fro')**2

        # Compute adaptive lambda
        current_lambda2 = self.lambda_2_max * min(1.0, (step / self.t0)) if step is not None else 0.0
        ...
        self.current_lambda2 = current_lambda2
...
def run_sae_training(
    ...
    trainer = TrainerTopK(
        ...
        k=120,
        auxk_alpha=1/32,
        alpha=0.0,  # Run 1: strict orthogonality
        lambda_2_max=0.1,
        t0=1000,
