...
class AutoEncoderTopK(nn.Module):
    ...
    def compute_mask_intersections(self, top_indices_BK):
        """Compute asymmetric competition statistics"""
        ...
        # Compute asymmetric co-occurrence matrix
        cooccurrence = torch.mm(masks.t(), masks)  # [F, F]
        total_activations = cooccurrence.diag()  # [F]
        
        # Compute directional competition coefficients
        k = self.k
        d = self.dict_size
        c_matrix = (cooccurrence / (total_activations.unsqueeze(0) + 1e-8)) / (k/d)
        
        return cooccurrence, c_matrix
...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        competition_weight=0.01,  # weight for hierarchical competition loss
    ...
    def loss(self, x, step=None, logging=False):
        ...
        cooccurrence, c_matrix = self.ae.compute_mask_intersections(top_indices)
        feature_dots = t.einsum('bi,bj->ij', f, f) / x.size(0)  # Normalize by batch size
        max_competition = t.maximum(c_matrix, c_matrix.t())
        competition_loss = (max_competition * feature_dots.pow(2)).mean()

        l2_loss = e.pow(2).sum(dim=-1).mean()
        auxk_loss = auxk_loss.sum(dim=-1).mean()
        loss = l2_loss + self.auxk_alpha * auxk_loss + self.competition_weight * competition_loss
