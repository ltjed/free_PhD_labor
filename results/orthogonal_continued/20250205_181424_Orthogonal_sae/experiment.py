...
class AutoEncoderTopK(nn.Module):
    ...
    def compute_orthogonality(self):
        """Compute orthogonality measure of encoder weights"""
        W = self.W_enc
        dot = torch.mm(W.T, W)
        identity = torch.eye(W.shape[1], device=W.device)
        return torch.norm(dot - self.alpha*identity, p='fro')

    def condition_number(self):
        """Compute condition number of encoder weight matrix"""
        with torch.no_grad():
            W = self.W_enc
            _, s, _ = torch.svd(W)
            return s[0]/s[-1] if s[-1] > 1e-6 else float('inf')
...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        submodule_name=None,
        ortho_max=0.1,  # λ2_max for adaptive ortho loss
        t0=1000,        # Ramp-up steps for ortho loss
        alpha=0.1,      # Controlled feature sharing parameter
    ):
        super().__init__(seed)
        ...
        self.logging_parameters = ["effective_l0", "dead_features", "condition_number"]
        self.effective_l0 = -1
    ...
    def loss(self, x, step=None, logging=False):
        ...
        # Compute adaptive orthogonality loss
        lambda_ortho = self.ortho_max * min(1.0, (step/self.t0)) if step else 0
        ortho_loss = self.ae.compute_orthogonality()
        ...
        self.condition_number = self.ae.condition_number().item()
