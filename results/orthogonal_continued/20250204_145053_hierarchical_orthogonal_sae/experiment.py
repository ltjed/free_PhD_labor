...
class AutoEncoderTopK(nn.Module):
    ...
    def compute_competition_coefficients(self, top_indices_BK):
        ...
        # c_ij = P(i active | j active) = n_ij / n_j normalized by k/d
        # Where n_ij is co-activation count and n_j is activation count of j
        cond_probs = coact_counts / (act_counts.t() + 1e-8)  # F x F
        
        # Normalize by expected random co-activation rate k/d
        expected_coact = k / F
        competition_coeff = cond_probs / expected_coact
...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        decay_start=None,  # when does the lr decay start
        ...
        # Ensure decay_start is properly initialized
        if decay_start is None:
            decay_start = int(0.8 * steps)  # Default to 80% of total steps
    ...
    def loss(self, x, step=None, logging=False):
        ...
        competition_coeff = self.ae.compute_competition_coefficients(top_indices)
        feature_dots = t.einsum('bi,bj->ij', f, f) / x.size(0)
        
        # Take max of c_ij and c_ji to penalize both directions of absorption
        symmetric_coeff = torch.maximum(competition_coeff, competition_coeff.t())
        
        # Competition loss penalizes feature similarity weighted by competition coefficient
        competition_loss = (symmetric_coeff * feature_dots.pow(2)).mean()
