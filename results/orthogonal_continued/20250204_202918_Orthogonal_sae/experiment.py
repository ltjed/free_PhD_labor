...
class AutoEncoderTopK(nn.Module):
    ...
    def compute_condition_number(self, features):
        """Compute condition number of feature subspace"""
        # SVD decomposition
        U, S, V = torch.linalg.svd(features.T @ features)
        # Condition number is ratio of largest to smallest singular value
        return S[0] / (S[-1] + 1e-8)
        
    def get_feature_groups(self, features, n_groups=32):
        """Group features based on activation patterns"""
        # Compute feature similarities
        sim_matrix = torch.mm(features.T, features)
        sim_matrix = sim_matrix / (torch.norm(features, dim=0).unsqueeze(0) + 1e-8)
        
        # Simple greedy grouping based on similarities
        groups = []
        unused = set(range(features.size(1)))
        
        while len(unused) > 0 and len(groups) < n_groups:
            # Pick random start feature
            start = unused.pop()
            group = {start}
            
            # Find most similar features
            sims = sim_matrix[start]
            while len(group) < features.size(1) // n_groups and len(unused) > 0:
                next_feat = max(unused, key=lambda x: sims[x])
                group.add(next_feat)
                unused.remove(next_feat)
...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        submodule_name=None,
        feature_sharing_alpha=0.1,  # Controls amount of allowed feature sharing
        n_feature_groups=32,  # Number of feature groups
        ortho_schedule='linear',  # Orthogonality loss schedule type
        ortho_warmup_steps=1000,  # Steps to warm up orthogonality loss
    ):
        super().__init__(seed)
        ...
        self.feature_sharing_alpha = feature_sharing_alpha
        self.n_feature_groups = n_feature_groups
        self.ortho_schedule = ortho_schedule
        self.ortho_warmup_steps = ortho_warmup_steps
        self.feature_groups = None
        self.group_condition_numbers = []
        ...
        def lr_fn(step):
            if step < decay_start:
                ...
                return max(0.0, (steps - step) / max(1e-8, steps - decay_start))
    ...
    def get_ortho_weight(self, step):
        """Get scheduled orthogonality loss weight"""
        if self.ortho_schedule == 'linear':
            return self.ortho_weight * min(1.0, step / self.ortho_warmup_steps)
        elif self.ortho_schedule == 'cosine':
            return self.ortho_weight * (1 - np.cos(np.pi * min(1.0, step / self.ortho_warmup_steps) / 2))
        elif self.ortho_schedule == 'step':
            return self.ortho_weight if step >= self.ortho_warmup_steps else 0.0
        return self.ortho_weight
    ...
    def loss(self, x, step=None, logging=False):
        ...
        if step % 100 == 0:
            self.feature_groups = self.ae.get_feature_groups(f, self.n_feature_groups)
            
        # Compute condition numbers for each group
        if self.feature_groups:
            for group in self.feature_groups:
                group_features = f[:, group]
                cond_num = self.ae.compute_condition_number(group_features)
                self.group_condition_numbers.append(cond_num.item())

        # Compute adaptive orthogonality loss with controlled sharing
        mask_intersections = self.ae.compute_mask_intersections(top_indices)
        feature_dots = t.einsum('bi,bj->ij', f, f) / x.size(0)
        sharing_target = t.eye(self.ae.dict_size, device=x.device) * self.feature_sharing_alpha
        ortho_loss = (mask_intersections * (feature_dots - sharing_target).pow(2)).mean()
        
        # Apply scheduled weight
        ortho_weight = self.get_ortho_weight(step)
...
def run_sae_training(
    ...
    trainer = TrainerTopK(
        ...
        auxk_alpha = 1/32,
        ortho_weight= 0.1,
        decay_start=int(steps * 0.8),  # Start decay at 80% of training
        steps=steps,
        seed=seed,
        device=device,
        layer=layer,
...
if __name__ == "__main__":
    ...
    num_tokens = 5_00 # Set default number of tokens, can be increased by a factor of up to 10 but takes much longer. Note training steps = num_tokens/sae_batch_size, so you can increase training be increasing num_of_tokens
