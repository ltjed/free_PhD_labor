...
class AutoEncoderTopK(nn.Module):
    ...
    def compute_mask_intersections(self, top_indices_BK):
        ...
        # c_ij = (n_i|j / n_j) / (k/d) where n_i|j is activations of i when j is active
        expected_overlap = k / F  # k/d term
        c_ij = intersections / (counts.t() + 1e-8) / expected_overlap  # F x F
        
        # Zero out diagonal
        c_ij.fill_diagonal_(0)
...
class TrainerTopK(SAETrainer):
    ...
    def loss(self, x, step=None, logging=False):
        ...
        # Take max of c_ij and c_ji for each pair to penalize both directions
        max_coeff = t.maximum(competition_coeff, competition_coeff.t())
        competition_loss = (max_coeff * feature_dots.pow(2)).mean()
