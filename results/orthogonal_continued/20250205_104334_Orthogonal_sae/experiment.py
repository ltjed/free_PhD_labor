...
class TrainerTopK(SAETrainer):
    ...
    def __init__(
        ...
        ortho_weight_max=0.01,  # max weight for orthogonality loss
        t0=1000,  # ramp-up period for ortho weight
    ...
    def loss(self, x, step=None, logging=False):
        ...
        W_dec = self.ae.W_dec
        feature_dots = t.mm(W_dec, W_dec.T)  # [d_sae, d_sae]
        
        # Create mask for off-diagonal elements
        mask = 1 - t.eye(feature_dots.size(0), device=feature_dots.device)
        
        # Calculate orthogonal loss with controlled sharing
        ortho_loss = ((feature_dots*mask - self.alpha*mask).pow(2).sum() / mask.sum())
        
        # Adaptive weight scheduling
        current_ortho_weight = self.ortho_weight_max * min(1, step/self.t0) if step is not None else self.ortho_weight_max
