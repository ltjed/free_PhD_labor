...
class StableSplitSAE(nn.Module):
    """Implementation of Stable-Split Orthogonal Sparse Autoencoder with EMA tracking."""
    def __init__(
        self,
        d_in: int,
        d_sae: int,
        ...
        # EMA tracking
        self.register_buffer('sparsity_ema', torch.zeros(d_sae))
    ...
    def encode(self, input_acts):
        ...
        # Update EMA of sparsity
        with torch.no_grad():
            sparsity = (acts == 0).float().mean(0)
            self.sparsity_ema.mul_(self.ema_decay).add_(sparsity * (1 - self.ema_decay))
...
class StableSplitTrainer(SAETrainer):
    ...
    def config(self):
        return {
            'trainer_class': 'StableSplitTrainer',
