...
class SelectiveOrthogonalSAE(nn.Module):
    """Implementation of Sparse Autoencoder with selective decoder orthogonality."""
    def __init__(
        self,
        d_in: int,
        d_sae: int,
        ...
        self.cfg = CustomSAEConfig(
            ...
            architecture="SelectiveOrthogonal",
...
class SelectiveOrthogonalTrainer(SAETrainer):
    """Trainer for Sparse Autoencoder with selective decoder orthogonality."""
    def __init__(self,
                 activation_dim=512,
                 dict_size=64*512,
                 lr=1e-3, 
        ...
        self.steps = steps
        self.resample_steps = int(steps * 0.3) if steps is not None else None
        ...
        self.tau = 0.3  # Similarity threshold
        self.l2_max = 0.1  # Max orthogonality weight
        self.warmup_frac = 0.2  # Warmup period as fraction of total steps
    ...
    def compute_ortho_loss(self, step):
        # Normalize decoder weights
        W_dec_norm = self.ae.W_dec / self.ae.W_dec.norm(dim=0, keepdim=True)
        # Compute pairwise similarities
        sim = torch.mm(W_dec_norm.t(), W_dec_norm)
        # Create similarity mask
        mask = (sim > self.tau).float() - torch.eye(sim.shape[0], device=sim.device)
        mask = mask * (mask > 0).float()
        # Compute loss
        ortho_loss = (sim * mask).norm(p='fro')
        # Apply warmup schedule
        warmup_steps = int(self.steps * self.warmup_frac)
        l2_weight = self.l2_max * max(0, 1 - step/warmup_steps)
        return ortho_loss * l2_weight
    ...
    def loss(self, x, step, logging=False, **kwargs):
        ...
        ortho_loss = self.compute_ortho_loss(step)
        ...
        loss = recon_loss + self.l1_penalty * l1_loss + ortho_loss
