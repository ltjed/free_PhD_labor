...
class CustomSAEConfig:
    ...
    ortho_penalty: float = 0.2  # τ in paper
    topk_percent: float = 0.001  # 0.1%
    normalization_type: str = "l2"  # Weight normalization type
...
class ConstrainedAdam(torch.optim.Adam):
    """Adam with L2 weight normalization for decoder weights."""
    def __init__(self, params, constrained_params, lr):
        super().__init__(params, lr=lr)
        self.constrained_params = list(constrained_params)
        # Register post-step hook for weight normalization
        self.register_post_step_hook(self._weight_norm_hook)
    
    def _weight_norm_hook(self, optimizer, args, kwargs):
        """Apply L2 normalization to decoder weights after step"""
        with torch.no_grad():
...
class CustomTrainer(SAETrainer):
    ...
    def loss(self, x, logging=False, **kwargs):
        ...
        # Calculate feature pair products and select top 0.1%
        with torch.no_grad():
            # Get upper triangle without diagonal
            triu_mask = torch.triu(torch.ones(f.shape[1], f.shape[1], device=f.device), diagonal=1).bool()
            feature_products = (f.T @ f).square()
            flat_products = feature_products[triu_mask]
            
            # Select top 0.1% pairs
            k = max(1, int(0.001 * flat_products.numel()))
            topk_values, topk_indices = torch.topk(flat_products, k)
            
        # Calculate orthogonality penalty using decoder weights
        W_dec = self.ae.W_dec
        row_idx, col_idx = torch.triu_indices(W_dec.shape[1], W_dec.shape[1], offset=1, device=W_dec.device)
        selected_pairs = (row_idx[topk_indices], col_idx[topk_indices])
        ortho_loss = (W_dec[:, selected_pairs[0]] * W_dec[:, selected_pairs[1]]).sum(dim=0).square().mean()
        ...
        # Total loss with orthogonality penalty (τ=0.2 fixed)
        loss = l2_loss + self.l1_penalty * l1_loss + 0.2 * ortho_loss
        return {"loss_for_backward": loss, "loss": loss.cpu().item(), 
                "l1_loss": l1_loss.cpu().item(), "l2_loss": l2_loss.cpu().item(),
...
def run_sae_training(
    ...
    results = {
        "training_log": special_slice(training_log, m = 10, num_k= 200),
    ...
    if os.path.exists(all_info_path):
        ...
        existing_data = {}
    existing_data.update({"training results" : results})
...
MODEL_CONFIGS = {
    ...
    "google/gemma-2-2b": {"batch_size": 32, "dtype": "bfloat16", "layers": [5], "d_model": 2304},
}
...
if __name__ == "__main__":
    ...
    num_tokens = 10_000_00 # Set default number of tokens, can be increased by a factor of 10 to 100 but takes longer
 ...
 warmup_steps=1000,
  seed=42,
    ...
    eval_types = [
        "absorption", # autointerp hasnt been tested
        "autointerp",
        "core",
        "scr_and_tpp",
        "sparse_probing",
