...
class CustomSAE(nn.Module):
    """Implementation of a Custom Sparse Autoencoder with EMA tracking."""
    def __init__(
        self,
        d_in: int,
        d_sae: int,
        ...
        self.register_buffer('feature_sim_m2', torch.zeros(d_sae))
    ...
    def encode(self, input_acts):
        ...
        with torch.no_grad():
            ...
            sim_matrix = torch.mm(acts.T, acts) / (norms.T @ norms + 1e-8)
            sim_matrix = torch.nan_to_num(sim_matrix)
            
            # Update Welford's algorithm for EMA and variance
            delta = sim_matrix.diag() - self.feature_sim_ema
            self.feature_sim_ema += (1 - self.ema_decay) * delta
            self.feature_sim_m2 += (1 - self.ema_decay) * (delta**2 - self.feature_sim_m2)
...
class SAETrainer:
    ...
    def get_logging_parameters(self):
        ...
        # Add EMA stats to logging
        if hasattr(self, 'feature_sim_ema'):
            stats['feature_sim_ema_mean'] = self.feature_sim_ema.mean().item()
            stats['feature_sim_ema_std'] = torch.sqrt(self.feature_sim_m2).mean().item()
            stats['feature_sim_min'] = self.feature_sim_ema.min().item()
            stats['feature_sim_max'] = self.feature_sim_ema.max().item()
...
if __name__ == "__main__":
    ...
    # Using Pythia-70m as it's fully open access
