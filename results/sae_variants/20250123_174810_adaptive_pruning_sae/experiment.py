...
class BitPackedMaskManager:
    def __init__(self, d_in: int, d_sae: int):
        self.d_in = d_in
        self.d_sae = d_sae
        self.masks = torch.ones(d_in, d_sae, dtype=torch.bool)
        self.correlation_cache = {}
        
    def update_masks(self, activations: torch.Tensor, threshold: float = 0.5):
        # Calculate correlations if not in cache
        batch_key = str(activations.shape)
        if batch_key not in self.correlation_cache:
            corr_matrix = F.normalize(activations.T) @ F.normalize(activations)
            self.correlation_cache[batch_key] = corr_matrix
        
        # Update masks based on correlations
        corr_matrix = self.correlation_cache[batch_key]
        self.masks = (corr_matrix < threshold)
        return self.masks

    def apply_masks(self, weights: torch.Tensor) -> torch.Tensor:
        return weights * self.masks.to(weights.device)
...
class AdaptiveSAE(nn.Module):
    ...
    def __init__(
        ...
        initial_pruning_rate: float = 0.2,
        final_pruning_rate: float = 0.5,
        correlation_threshold: float = 0.5,
        pruning_schedule_steps: int = 1000
    ):
        super().__init__()
        # Initialize mask manager
        self.mask_manager = BitPackedMaskManager(d_in, d_sae)
    ...
    def encode(self, input_acts):
        # Apply masks during encoding
        masked_W_enc = self.mask_manager.apply_masks(self.W_enc)
    ...
    def forward(self, acts, output_features=False):
        # Calculate current pruning rate based on schedule
        if self.current_step < self.pruning_schedule_steps:
            current_pruning_rate = self.initial_pruning_rate + (self.final_pruning_rate - self.initial_pruning_rate) * (self.current_step / self.pruning_schedule_steps)
        else:
            current_pruning_rate = self.final_pruning_rate
        
        # Update masks based on current activations and pruning rate
        self.mask_manager.update_masks(acts, self.correlation_threshold * current_pruning_rate)
...
def run_sae_training(
    ...
    try:
        activation_buffer = ActivationBuffer(
            ...
            model,
            submodule,
            n_ctxs=16,  # Further reduced buffer size
            ctx_len=context_length,
            refresh_batch_size=4,  # Smaller refresh batch size 
            out_batch_size=8,  # Smaller output batch size
            io="out",
    ...
    for step in range(steps):
        try:
            ...
            activations = activations.to(device)  # Ensure proper device placement
        except Exception as e:
...
if __name__ == "__main__":
    ...
    for layer in layers:
        saes.append(run_sae_training(
            ...
            buffer_size=2048,
            llm_batch_size=llm_batch_size,
            sae_batch_size=8,  # Match the reduced buffer out_batch_size
            learning_rate=3e-4,
