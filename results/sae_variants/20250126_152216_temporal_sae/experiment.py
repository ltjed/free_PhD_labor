...
class TemporalSAE(nn.Module):
    """Implementation of Temporal SAE with positional masking."""
    def __init__(
        self,
        d_in: int,
        d_sae: int,
        ...
        # Initialize weights with positional masking
        self.W_enc = nn.Parameter(torch.zeros(d_in, d_sae))
        self.W_dec = nn.Parameter(torch.zeros(d_sae, d_in))
        
        # Create position-specific masks
        features_per_position = d_sae // window_size
        mask = torch.zeros_like(self.W_enc)
        for pos in range(window_size):
            start_idx = pos * features_per_position
            end_idx = (pos + 1) * features_per_position
            mask[:, start_idx:end_idx] = 1.0
            
        # Initialize weights with masks
        nn.init.kaiming_uniform_(self.W_enc, nonlinearity='relu')
        self.W_enc.data *= mask
    ...
    def encode(self, input_acts):
        # Apply positional mask during forward pass
        masked_W_enc = self.W_enc * self.position_mask
