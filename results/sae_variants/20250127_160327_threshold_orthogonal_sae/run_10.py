...
class ThresholdOrthogonalTrainer(SAETrainer):
    ...
    def loss(self, x, logging=False, **kwargs):
        ...
        with torch.enable_grad():
            f_detached = f.detach().requires_grad_()
            x_hat_grad = self.ae.decode(f_detached)
            recon_loss = torch.linalg.norm(x - x_hat_grad, dim=-1).mean()
            feature_grads = torch.autograd.grad(recon_loss, f_detached, allow_unused=True)[0]
            importance_scores = feature_grads.abs().mean(dim=0) if feature_grads is not None else torch.ones_like(f[0])
            importance_scores = importance_scores / (importance_scores.max() + 1e-6)

        # Compute feature correlations
        W_dec_norm = self.ae.W_dec / self.ae.W_dec.norm(dim=0, keepdim=True)
        ortho_matrix = torch.mm(W_dec_norm.t(), W_dec_norm)
        feature_correlations = ortho_matrix.abs()
        
        # Compute adaptive learning rates based on importance
        importance_based_lr = importance_scores.unsqueeze(0) * importance_scores.unsqueeze(1)
        importance_based_lr = importance_based_lr / importance_based_lr.max()
        
        # Scale orthogonality penalty inversely with correlation
        correlation_scaling = 1.0 / (feature_correlations + 1e-6)
        correlation_scaling = correlation_scaling / correlation_scaling.max()
        
        # Combine frequency and importance scores with correlation awareness
        freq_scores = self.ae.activation_frequency / self.ae.activation_frequency.max()
        combined_scores = (freq_scores + importance_scores) / 2
        
        # Dynamic threshold with correlation consideration
        feature_stats = torch.stack([freq_scores, importance_scores, correlation_scaling.mean(dim=1)])
        feature_variance = feature_stats.var(dim=0)
        dynamic_threshold = self.ae.activation_threshold * (1 - feature_variance)
        
        # Apply adaptive masking
        adaptive_mask = torch.outer(combined_scores, combined_scores)
        mask_matrix = (freq_scores > dynamic_threshold).float()
        ...
        penalty_matrix = (
            ...
            correlation_scaling * 
            importance_based_lr * 
            (1 - torch.eye(self.ae.dict_size, device=self.device))
        )
