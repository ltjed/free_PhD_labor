...
class TemporalTrainer(SAETrainer):
    ...
    def loss(self, x, logging=False, **kwargs):
        ...
        if self.ae.activation_history is not None:
            # Update activation history
            self.ae.activation_history = torch.roll(self.ae.activation_history, -1, dims=0)
            self.ae.activation_history[-1] = f.mean(dim=0)
            
            # Compute normalized temporal consistency with numerical stability
            eps = 1e-8
            window_mean = self.ae.activation_history.mean(dim=0, keepdim=True)
            window_diff = self.ae.activation_history - window_mean
            temporal_loss = (window_diff.pow(2).sum(dim=0) / (self.window_size - 1 + eps)).mean()
        ...
        # Weighted loss with normalized temporal component
        loss = l2_loss + self.l1_penalty * l1_loss + (self.temporal_penalty/self.window_size) * temporal_loss
...
def run_sae_training(
    ...
    trainer = TemporalTrainer(
        ...
        temporal_penalty=0.1,  # Lambda for temporal consistency
        window_size=8,        # Window size for temporal consistency
