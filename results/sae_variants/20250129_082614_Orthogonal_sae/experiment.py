...
class OrthogonalSAE(nn.Module):
    """Implementation of an Orthogonal Sparse Autoencoder with controlled feature sharing."""
    def __init__(
        self,
        d_in: int,
        d_sae: int,
        ...
        self.n_groups = 8  # Number of feature groups
...
class OrthogonalTrainer(SAETrainer):
    ...
    def __init__(self,
        ...
        self.alpha = 0.0  # Strict orthogonality
    ...
    def loss(self, x, logging=False, **kwargs):
        ...
        W = self.ae.W_enc  # [d_in, d_sae]
        W_normed = W / (W.norm(dim=0, keepdim=True) + 1e-8)
        gram = torch.mm(W_normed.t(), W_normed)  # [d_sae, d_sae]
        
        # Create target matrix with alpha on diagonal
        I = torch.eye(W.shape[1], device=W.device)
        target = I * self.alpha
        
        # Orthogonality loss excludes diagonal terms
        ortho_loss = torch.norm(gram - target, p='fro')
        ...
        loss = recon_loss + self.l1_penalty * l1_loss + self.ortho_scale * ortho_loss
        # Calculate condition number
        _, S, _ = torch.svd(W_normed)
        ...
        return {
            ...
            "condition_number": condition_number.cpu().item()
        }
        if not logging:
            return loss
        else:
