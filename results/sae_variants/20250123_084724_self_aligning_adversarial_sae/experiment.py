...
class CustomSAE(nn.Module):
    ...
    def __init__(
        ...
        self.W_adv = nn.Parameter(torch.zeros(d_sae, d_sae))  # Adversarial projection
        self.b_enc = nn.Parameter(torch.zeros(d_sae))
        self.b_dec = nn.Parameter(torch.zeros(d_in))
        self.margin = nn.Parameter(torch.ones(d_sae), requires_grad=False)  # Dynamic margin
        self.ema_sim = nn.Parameter(torch.zeros(d_sae), requires_grad=False)  # EMA similarity
        self.ema_std = nn.Parameter(torch.zeros(d_sae), requires_grad=False)  # EMA std dev
        self.attribution_percentile = nn.Parameter(torch.zeros(1), requires_grad=False)  # Moving percentile
...
class CustomTrainer(SAETrainer):
    ...
    def loss(self, x, logging=False, **kwargs):
        ...
        # Adversarial component with stop gradient
        with torch.no_grad():
            h_spurious = f @ self.ae.W_adv
        adv_loss = torch.relu(self.ae.margin - (f * h_spurious).sum(dim=-1)).mean()
        
        # Update EMA statistics
        sim = (f * h_spurious).sum(dim=-1)
        self.ae.ema_sim.data = 0.95 * self.ae.ema_sim + 0.05 * sim.mean(dim=0)
        self.ae.ema_std.data = 0.95 * self.ae.ema_std + 0.05 * sim.std(dim=0)
        self.ae.margin.data = self.ae.ema_sim + 2 * self.ae.ema_std
        ...
        # Combined loss with adaptive scaling
