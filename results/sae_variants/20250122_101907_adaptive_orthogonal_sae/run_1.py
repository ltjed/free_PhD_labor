...
class CustomSAE(nn.Module):
    """Implementation of an Adaptive Orthogonal Sparse Autoencoder."""
    def __init__(
        self,
        d_in: int,
        d_sae: int,
        ...
        alpha: float = 0.1,  # Sharing control parameter
        group_interval: int = 100,  # Steps between grouping updates
    ):
        # Add orthogonal learning params
        self.alpha = alpha
        self.group_interval = group_interval
        self.current_grouping = torch.zeros(d_sae, device=self.device)  # Track feature groups
        super().__init__()
...
class CustomTrainer(SAETrainer):
    ...
    def loss(self, x, logging=False, **kwargs):
        ...
        # Adaptive orthogonality loss with controlled sharing
        W = self.ae.W_enc
        gram_matrix = torch.matmul(W.T, W)
        identity = torch.eye(W.size(1), device=W.device)
        ortho_loss = torch.norm(gram_matrix - identity)**2
        ortho_loss = self.ae.alpha * ortho_loss
        
        # Feature grouping penalty
        if step % self.ae.group_interval == 0:
            self.ae.group_features()
            group_penalty = self._calculate_group_sparsity(f)
        else:
            group_penalty = 0
            
        total_loss = l2_loss + self.l1_penalty * l1_loss + ortho_loss + group_penalty
...
def run_sae_training(
    ...
    trainer = CustomTrainer(
        ...
        # New params for ortho learning
        alpha=0.1,  # Controlled sharing parameter
        group_interval=100  # Update groups every 100 steps
    )
