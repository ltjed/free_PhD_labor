...
@dataclass
class FixedOrthoSAEConfig:
    model_name: str
    d_in: int
    d_sae: int
    hook_layer: int
    hook_name: str
    # Ortho regularization params
    ortho_alpha: float = 10.0  # Fixed strong regularization
    group_size: int = 1  # No grouping
    group_update_interval: int = 0  # Disabled
...
class FixedOrthoSAE(nn.Module):
    """Sparse Autoencoder with Fixed Orthogonal Regularization."""
    def __init__(
        self,
        d_in: int,
        d_sae: int,
        ...
        ortho_alpha: float = 10.0,
        group_size: int = 1,
        group_update_interval: int = 0,
    ):
        super().__init__()
        ...
        self.cfg = FixedOrthoSAEConfig(
            ...
            group_update_interval=group_update_interval,
            architecture="FixedOrtho",
...
class FixedOrthoTrainer(SAETrainer):
    """Trainer for Fixed Orthogonal SAE with L1 + Ortho regularization."""
    def __init__(self,
                 activation_dim=512,
                 dict_size=64*512,
                 lr=1e-3, 
    ...
    def loss(self, x, logging=False, **kwargs):
        ...
        ortho_loss = torch.norm(self.ae.W_enc.T @ self.ae.W_enc - torch.eye(self.ae.d_sae, device=self.ae.device))**2
        loss = l2_loss + self.l1_penalty * l1_loss + self.ae.cfg.ortho_alpha * ortho_loss
